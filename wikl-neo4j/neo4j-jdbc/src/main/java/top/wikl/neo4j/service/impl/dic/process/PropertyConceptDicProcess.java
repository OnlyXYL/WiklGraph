...
/**
 * @author XYL
 * @title: PropertyConceptDicProcess
 * @description: TODO
 ...
 */
@Service
public class PropertyConceptDicProcess extends AbstractDicHandler<GetDicInput, GetDicOutPut> {

    @Resource
    private Neo4jUtils neo4jUtils;

    public PropertyConceptDicProcess() {
        super(DicType.property_concept_dic.getKey());
    }

    @Override
    public boolean support(String id) {

        if (DicType.property_concept_dic.getKey().equals(id)) {
    ...
    public GetDicOutPut dic(GetDicInput param) throws Exception {
        ...
        //处理结果
        final Set<String> set = Neo4jResult.propertyConceptDic(result, labelMarkMap);

        String[] conceptNames = set.stream().toArray(String[]::new);

        //调用工具类
        FileUtil.writeStrToFile(conceptNames, param.getFilePath() + DicType.property_concept_dic.getValue() + "." + param.getSuffix());
